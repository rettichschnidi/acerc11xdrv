...
namespace DANGER_ZONE
{
	...
	std::list<Device *>
	USB::match(DeviceIDList devList)
	{
		std::list<Device *> match_list;
		std::list<Device *>::const_iterator diter;
	...
	int
	Device::string(std::string &buf, int index, u_int16_t langID)
	{
		int retval;
	...
	int
	Device::controlTransfer(u_int8_t requestType, u_int8_t request,
			...
			Device * device)
	{
		...
		for (uint8_t i = 0; i < m_config_descriptor->bNumInterfaces; i++)
			{
				push_back(new Interface(m_config_descriptor->interface[i], i,
						device));
			}
	}

	u_int8_t
	Configuration::numInterfaces(void)
	{
		return m_config_descriptor->bNumInterfaces;
	}

	void
	Configuration::dumpDescriptor(void)
	{
			...
			Device * device)
	{
		...
		for (uint8_t i = 0; i < m_interface_descriptor.bNumEndpoints; i++)
			{
				push_back(new Endpoint(m_interface_descriptor.endpoint[i], i,
						device));
			}
	}

	void
	AltSetting::dumpDescriptor(void)
	{
	...
	int
	Endpoint::bulkWrite(unsigned char *message, int size, int &wrote,
			int timeout)
	{
		int ret = libusb_bulk_transfer(m_parent->handle(), (m_number
				| LIBUSB_ENDPOINT_OUT), message, size, &wrote, timeout);
		if (ret)
			{
				return ret;
			}
	...
	int
	Endpoint::bulkRead(int length, unsigned char *message, int size, int &read,
			int timeout)
	{
		unsigned char *buf;
		int res;
		...
		res = libusb_bulk_transfer(m_parent->handle(), (m_number
				| LIBUSB_ENDPOINT_IN), buf, length, &read, timeout);
	...
	int
	Endpoint::clearHalt(void)
	{
		return libusb_clear_halt(m_parent->handle(), m_number);
	}

	void
	Endpoint::dumpDescriptor(void) const
	{
	...
	u_int16_t
	DeviceID::vendor(void) const
	{
		return m_vendor;
	}

	u_int16_t
	DeviceID::product(void) const
	{
		return m_product;
	}
}
