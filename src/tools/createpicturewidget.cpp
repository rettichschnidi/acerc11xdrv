...
int main(int argc, char **argv) {
	...
	try {
		desc.add_options()("source,s", boost::program_options::value<string>(), "JPEG file to read")
				("destination,d", boost::program_options::value<string>(), "Where to write the widget. Default is source + \".widget\"")
		...
		posOpt	.add("source", 1)
				.add("destination", 1)
				.add("overwriteDestinationIfExisting", 1);
	...
	destination	= variablesMap.count("destination") ? variablesMap["destination"].as<string> () : source + ".widget";

	try {
		PictureWidgetUSBData data(file_size(source));
